...
use iter::consecutive_token_iter;
...
pub use trainer::data::PunktData as Data;
...
impl<T> Collocation<T> {
  #[inline]
  pub fn new(left: T, right: T) -> Collocation<T> {
    Collocation {
      left: left,
      right: right
    }
...
impl Eq for Collocation<Rc<Token>> { }

impl Hash<XXState> for Collocation<Rc<Token>> {
  #[inline]
  fn hash(&self, state: &mut XXState) {
    (*self.left).typ_without_period().hash(state); 
    (*self.right).typ_without_break_or_period().hash(state);
  }
}

impl PartialEq for Collocation<Rc<Token>> {
  #[inline]
  fn eq(&self, other: &Collocation<Rc<Token>>) -> bool {
    (*self.left).typ_without_period() == (*other.left).typ_without_period() &&
    (*self.right).typ_without_break_or_period() == (*other.right).typ_without_break_or_period()
  }
}

/// Internal type used by trainer. 
type TrainerCollocation = Collocation<Rc<Token>>;
...
mod punkt_trainer_bench {
  ...
  #[bench]
  fn punkt_trainer_bench_short(b: &mut Bencher) {
    let tokenizer = WordTokenizer::new();
    let mut data = Default::default();
    let mut trainer = Trainer::with_data(&mut data);

    b.iter(|| {
      let tokens = tokenizer.tokenize_document(
        include_str!("../../test/sigma-wiki-raw.txt")); 
  ...
  #[bench]
  fn punkt_trainer_bench_very_long(b: &mut Bencher) {
    let tokenizer = WordTokenizer::new();
    let mut data = Default::default();
    let mut trainer = Trainer::with_data(&mut data);
    ...
    b.iter(|| {
      let tokens = tokenizer.tokenize_document(
        include_str!("../../test/pride-and-prejudice-raw.txt")); 
